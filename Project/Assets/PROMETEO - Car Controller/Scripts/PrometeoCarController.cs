...
public class PrometeoCarController : MonoBehaviour
{
    ...
    */
    ...
    public bool AiWantsToGoForward;
    public bool AiWantsToGoBackward;
    public bool AiWantsToTurnRight;
    public bool AiWantsToTurnLeft;

    //CAR DATA

    [HideInInspector] public float carTyreSpeed; // Used to store the speed of the car.

    [HideInInspector]
    public float carBodySpeed => carRigidbody.linearVelocity.magnitude; // Used to store the speed of the car's body.
    ...
    */
    ...
    void Update()
    {
        ...
        carTyreSpeed = (2 * Mathf.PI * frontLeftCollider.radius * frontLeftCollider.rpm * 60) / 1000;
        ...
        */
        ...
        {
            bool w = Input.GetKey(KeyCode.W) || AiWantsToGoForward;
            bool s = Input.GetKey(KeyCode.S) || AiWantsToGoBackward;
            bool a = Input.GetKey(KeyCode.A) || AiWantsToTurnLeft;
            bool d = Input.GetKey(KeyCode.D) || AiWantsToTurnRight;
            ...
            if ((!s && !w) && !Input.GetKey(KeyCode.Space) &&
                !deceleratingCar)
            {
                InvokeRepeating("DecelerateCar", 0f, 0.1f);
                deceleratingCar = true;
        ...
        if (useUI)
        {
            float absoluteCarSpeed = Mathf.Abs(carBodySpeed * 3.6f / 3f);
            float directionMul = Mathf.Sign(Vector3.Dot(carRigidbody.linearVelocity, transform.forward));
            carSpeedText.text = (absoluteCarSpeed * directionMul).ToString("00.000", CultureInfo.InvariantCulture);
        }
    }
    ...
    public void CarSounds()
    {
        if (useSounds)
        {
            try
            {
                ...
                if ((isDrifting) || (isTractionLocked && Mathf.Abs(carTyreSpeed) > 12f))
                {
                ...
                else if ((!isDrifting) && (!isTractionLocked || Mathf.Abs(carTyreSpeed) < 12f))
                {
                    tireScreechSound.Stop();
                }
            }
    ...
    public void GoForward()
    {
        ...
        {
            if (Mathf.RoundToInt(carTyreSpeed) < maxSpeed)
            {
    ...
    public void GoReverse()
    {
        ...
        {
            if (Mathf.Abs(Mathf.RoundToInt(carTyreSpeed)) < maxReverseSpeed)
            {
    ...
    public void DriftCarPS()
    {
        if (useEffects)
        {
            ...
            try
            {
                if ((isTractionLocked || Mathf.Abs(localVelocityX) > 5f) && Mathf.Abs(carTyreSpeed) > 12f)
                {
